...
async function handleSubmit() {
  ...
  try {
    // TODO: Implement API call when backend supports custom views
    // For now, show a message that this feature is coming soon
    error.value = 'Custom views feature coming soon!'
...
<template>
  <Modal :open="open" @close="emit('close')" size="md" title="Create view">
    <form @submit.prevent="handleSubmit" class="space-y-4">
      ...
      <div>
        <label class="block text-xs font-medium text-gray-400 mb-1">View name</label>
        <Input
          v-model="name"
          type="text"
