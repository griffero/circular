...
| Tier 1 functional/filter/sub-view checklist >= 95% per view | PARTIAL | `spec/requests/issues_spec.rb`, `spec/requests/users_spec.rb`, `PROGRESS.md`, `parity/SCORE_HISTORY.md` | Team issue-shell tabs now carry filter query state into Board and Team Board now hydrates/persists those filters through route query sync. Remaining score gaps to `95`: functional `0`, filter `2`, sub-view `2`. |
...
| Tier 1 critical data/order cases 100% match; remaining >=95% | PASS | `spec/requests/issues_spec.rb` | Covered: default order, explicit `updated_at asc`, priority/due-date ordering, workflow-state and status filters. |
...
- Filter parity: `93` (`gap: 2`)
- Sub-view parity: `93` (`gap: 2`)
- Data/order parity: `93` (`gap: 2`)
